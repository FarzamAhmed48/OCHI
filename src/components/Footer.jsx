...
const Footer = () => {
  return (
    <div className='w-full h-full bg-zinc-900  py-20 flex gap-5 '>
        <div className="w-1/2 flex flex-col px-20 h-full justify-between">
            <div className='headings '>
                <h1 className='uppercase text-[8vw] font-semibold tracking-tighter leading-none -mb-15'>Eye-</h1>
                <h1 className='uppercase text-[8vw] font-semibold tracking-tighter leading-none -mb-15'>Opening</h1>
            </div>
        ...
        <div className="w-1/2">
            <div className='w-full '>
                <h1 className='uppercase text-[5vw] font-semibold tracking-none leading-none '>Presenatations</h1>
            </div>
            <div className='w-full h-1/2 mt-10 bg-zinc-900 flex gap-[]'>
                <div className='bg-zinc-900'>
                    {["S:","Instagram","Behance","Facebook","LinkedIn"].map((item,index)=>(
                        <p className={`${index===0 && "mb-5 no-underline"  } underline`}>{item}</p>
                    ))}
                    {["L:","202-1965 W 4th Ave","Vancouver, Canada","30 Chukarina St","Lviv, Ukraine"].map((item,index)=>(
                        <p className={`${index===0 && "mt-10 mb-5 no-underline"} underline`}>{item}</p>
                    ))}
                    {["E:","hello@ochi.design"].map((item,index)=>(
                        <p className={`${index===0 && "mb-5 no-underline mt-10"  } underline`}>{item}</p>
                    ))}
                    <p className='mt-16'>&copy;ochi design 2024. <span className='underline '>Legal Terms</span></p>
                </div>
                <div>
                {["M:","Home","Services","Our Work","About us"].map((item,index)=>(
                        <p className={`${index===0 && "mb-5 no-underline"  } underline`}>{item}</p>
                    ))}
